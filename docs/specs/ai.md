...
| ID | 场景 | 期望 |
|----|------|------|
| S1 | 跨章人物/主题 | 多步工具 → 分块回答 → 芯片续聊 |
| S2 | 合集当前部 vs 整本文件 | 默认限当前部；切「整本文件」后可跨部检索 |
| S3 | 本书书摘任务 | 范围说明 + 冲突/人物/主题结构 |
| S4 | 本章导图快捷 / 明确生成 | 轻路径出原生图，无确认卡；禁止 Markdown「二、思维导图」伪交付 |
| S5 | 文末邀约 + 芯片/「可以」 | 可接精读或导图，否定不误触发 |
...
- 桌面输入框：Enter 在**没有输入法组合文本**时发送，Shift+Enter 换行；组合输入期间（中文、日文、韩文等候选词确认）一律交回系统 `EditableText` 处理，不另设并行的 Focus Enter 回调。键盘动作与发送按钮必须汇入同一个互斥提交入口，提交前冻结文本并结束编辑连接；同一平台 action 即使重复到达也只能创建一个 turn。手机仍使用键盘的发送动作。
