...
- 本书 AI 工作区提供对话 / 知识图谱两个 Tab；大纲与思维导图都从对话自然语言或快捷入口发起。思维导图请求由 App 识别“当前章/本章”与“这本书/全书”范围，冻结后只路由到独立 `BookMindMapWorkflow`，结果作为结构化对话附件显示，不向聊天 Agent 发送长期任务提示词。
...
知识图谱的辅文、关系、别名与书名先验属于高级算法规则，普通设置页只展示「分析未读内容」和一个「高级图谱规则」入口。规则在独立页面内显式保存；切换服务商、切换总开关或离开普通设置页不得提交未保存的规则草稿。「放弃修改」必须恢复到进入规则页时的已保存值。
...
- 文件内部结构由共享的 `BookStructureManifest` 表达，至少区分：普通单本、单本分部/分卷、多作品合订和证据不足。书库「合集」只是多个独立 `ReadingItem` 的外层整理关系，成员进入阅读器后仍按自己的文件结构处理；不得把书库合集和文件内部合订混为一种类型。
- 结构识别只消费阅读器提供的确定性事实（TOC 层级、导航目标、spine、heading 与逻辑单元），不让 AI 判断书型。TOC 层级明确时按导航锚点建立作品范围；证据冲突或同一 spine 内无法定位当前作品时标记为不确定，对话回退整个文件，大纲/图谱继续由用户显式确认范围。内部层级仅用于范围定位，不恢复树形大纲 UI。
