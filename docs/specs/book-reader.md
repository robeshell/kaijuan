...
① 默认露出：划线 · 笔记 · 复制 · 词典 · 翻译 · 书摘（书摘置末）。无后端的槽可短提示，由产品点将决定是否隐藏。
...
- 跨章连续选；追像素复刻微信；漫画选区；无引擎的假词典/翻译按钮。
