...
- 书内超链接：由 Foliate 处理 spine/fragment；外链的系统打开策略待 bridge 接入。
...
- 外链 bridge 当前尚未接到系统打开能力；接入前不得把 noop handler 写成“已支持”。
...
- **打开时序**：点击图书后创建一次 WebView，loopback server 流式提供 foliate-js 与 EPUB，按 Anx Reader 的 File/zip.js 路径初始化；保存的 CFI 在首屏 init 时直接传入。controller 完成 TOC/section attach 前不进入 ready，新的打开任务或 dispose 必须使旧异步结果失效。
- **页面上下留白**：Chrome 为覆盖式悬浮层，不永久占用完整操作栏高度；正文只保留系统安全区 + 8dp 阅读留白。Chrome 显隐不得改变分页尺寸或页码。
- **滚动（仅 iOS / iPadOS / Android）**：同一 foliate Paginator 切换为 `flow=scrolled`，位置仍由 CFI 表示；切模式和尺寸变化不销毁 WebView。
