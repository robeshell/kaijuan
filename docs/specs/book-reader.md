...
| `@font-face` / 嵌入字体 | 打开书时注册；滚动/翻页读 `font-family` | 同左 |
...
- **滚动**：按**已测章节高度**映射 offset ↔ `BookLocator`；节 HTML **按需加载**；pending jump 失败须重试直至 layout 就绪。OPF + 节级 CSS 在 `HtmlSectionView` 以 `<style>` 注入；用户字号/行距/主题仍通过 `Style` 覆盖 `body` 基准。  
- **翻页**：渐进分页；大书（spine > 80）只排当前节窗口，页末可跨到下一节再排。未排到的节不可用 pageMap clamp 跳转，应保留 pending。`BookCssRules` 将 OPF/节级 CSS 映射为 class/标签粗体、缩进、标题倍率；**非**完整 CSS 引擎（见上表）。  
