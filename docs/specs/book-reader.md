...
| `@font-face` | 仅用户字体（下载/导入）写入 `fontPath`；系统栈与「图书自带」不伪造空 face |
...
- 预览：摘文 · 章节 · 书名（无作者字段则省略）；水印 **开卷**。
- **排版** 3 种（书页式 / 左栏编辑式 / 引号框式）；三种分别采用居中书页层级、左侧色条正文和带开闭引号的摘文框，不只是调整对齐方式；**配色·底色** 4～5 套纯色（无照片底）。
...
移动端 dismiss 屏障：点空白关菜单；若落在左右约 **28%** 翻页区，**同一次点按**顺带翻页（避免关菜单后还要点第二次）。  
...
- 弹层样式：圆角实色 surface（跟阅读主题底色，**无**毛玻璃 / backdrop-blur）；`book.js` 打开时写入 `backgroundColor`。
- 弹层正文：**不**继承阅读页段首缩进与侧边 `gap`；强制清作者横向缩进与段首全角空格。`#footnote-dialog` / `main` 使用 `box-sizing: border-box`，内边距约 `16×20`，避免 `width:100%`+padding 把字裁到圆角边；打开时先占位再 `render`，按真实弹层宽度换行。
...
- **点按**：翻页模式左右约 **28%** 翻页、中间 **44%** 显隐 chrome；**chrome 已显示时正文空白点按只关 chrome**（边缘也不翻页）。WebView 内链、表单控件与文字选择优先于空白点按。滚动模式点空白显隐 chrome。
...
- **正文字体**：`BookFontSelection`（book / system / user）→ Foliate `fontName` + 可选 `fontPath`。不做在线字体商店；「补充字体」入口不做。
...
- 路由：`EpubImportRouter` + `EpubKindProbe`（Dart，无 WebView）自动探测 — 正文 EPUB → book import；页图 EPUB → comic import。须在 spine 中均匀抽样，并以实际抽样节数计算文本密度；不能只采开头后除以全书节数，避免套装书被插图误判成漫画。图书元数据/封面仍用 Foliate probe。
