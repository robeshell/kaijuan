...
每套候选必须携带策略名、结构证据、拒绝原因、分数和完整范围。锚点缺失、范围交叉、声明数量冲突、为凑数量而猜测合并属于硬拒绝，不能被软分数抵消。软分数只比较已经满足硬约束的方案，参考章节序号重置、子树完整性、重复目录形态、正文覆盖、结构一致性与出版物级合集证据。得分相近但范围不同的方案不得自动决策，应返回 `uncertain` 并保留候选供对话确认。
...
- 明确册数与所有可定位方案均冲突时必须保持 `uncertain`；不得通过删除、拼接或平均分配节点凑数。
