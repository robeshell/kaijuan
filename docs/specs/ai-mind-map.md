...
- 支持缩放、拖动、重新居中、折叠/展开分支、节点详情和从证据跳回原文。对话卡片提供“全屏查看”，以独立 `Scaffold` 路由铺满 App 工作区；它共享同一结构化数据和布局选择回调，不新增 Tab 或第二份缓存。
...
- UI 可切换三种布局、折叠、缩放、拖动、打开详情并跳回证据；对话卡片可进入并关闭全屏画布，且全屏中的布局切换会保存回同一对话附件。
