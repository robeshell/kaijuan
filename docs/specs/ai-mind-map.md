...
- 下一阶段将把上述范围结果升级为类型化 `AiConversationIntent`：`object=mindMap` 下区分 `create / edit / regenerate / export / discuss`，并通过会话上下文解析“这张/上一张/刚才那张”结构化附件。当前没有编辑 Workflow 时，“修改这张思维导图”继续普通对话；编辑能力上线后，缺少具体编辑要求必须在对话内追问，不能静默重新生成。
