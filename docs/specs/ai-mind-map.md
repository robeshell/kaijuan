...
每次生成前复用 `AiBookStructureSession` 的确定性结果，并向用户展示即将处理的书名、章节数以及作品/分卷数量，不能在没有任何反馈时直接开始模型调用。
...
- `multiWorkOmnibus`：合集请求先展示“共 N 部作品、M 章”的范围选择。用户可选当前作品、任一作品或“全部作品”；全部作品按作品顺序依次生成，每部一张卡片。
- `uncertain`：存在可展示的候选作品时同样请求用户选择；没有可靠候选时明确告知将按整本出版物处理，不伪装成已识别的合集。
- 当前章正文直接来自 Foliate 当前文档，并在异步调用前冻结标题、正文和物理 section。作品/分卷范围来自共享结构事实，不另建思维导图识别器。

范围选择只解决“生成哪一部”，不提供知识图谱式章节逐项勾选。取消选择不会产生模型请求。
...
- 节点数量、分支数量和层级深度由内容决定。标题用于浏览，摘要必须写出正文中的观点、事实、例子、过程、影响或结论，不能只重排目录。
...
多作品选择属于生成前预检，不创建独立思维导图页。逐作品/逐卷生成时，进度文案显示当前序号、范围标题与章节数；每个已完成范围立即成为对话卡片，后续范围失败不得删除前面已经完成的卡片。

## 8. 验收

- “生成本章思维导图”完整发送冻结章节与用户原始要求，只调用一次结构化模型；普通单书全书同样一次调用。
- 分卷单书按卷依次生成；多作品合集在任何模型调用前展示作品数、章节数与范围选择，选择全部后按作品依次生成。
