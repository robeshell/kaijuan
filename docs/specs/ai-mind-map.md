...
- 自然语言路由接受两类明确的“获取思维导图”意图：一类是生成、重新生成、创建、制作、绘制、画、做、整理成和梳理成等创作动作；另一类是需要、想要、给我、请给、来一份和想看等获取表达。范围词与请求意图独立解析，因此“我需要这本书的思维导图”直接进入全书 Workflow。出现不要、不需要、无需、别等否定表达，明确要求 Mermaid，询问为什么/如何制作，评价或修改已有导图，或比较概念时继续普通对话。
- 上述范围结果已升级为类型化 `AiConversationIntent`：`object=mindMap` 下当前区分 `create / edit / regenerate / discuss`，并为 `export` 预留同一目标槽位；通过会话上下文解析“这张/上一张/刚才那张”结构化附件。书籍对话已启用编辑 Workflow；未启用该能力的其他调用方仍可显式回落普通对话。缺少具体目标必须在对话内追问，不能静默重新生成。
