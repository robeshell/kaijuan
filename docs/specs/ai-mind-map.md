...
结构预检必须先排除章/篇标题及破折号开头的连续副标题；这些标题共享一个物理 spine 或导航目标是常见 EPUB 排版，不能作为“多作品合订”的证据。只有剩余的非章节顶层容器形成至少两个独立范围时，才展示作品选择卡片。
...
- `uncertain`：存在可展示的候选作品时同样在对话内请求用户选择；没有可靠候选时明确告知将按整本出版物处理，不伪装成已识别的合集。
...
- 节点数量、分支数量和层级深度由内容决定，并由 App 根据有效章节数与正文字符数给出动态覆盖目标：全书至少覆盖所有实质章节，每章通常产生一个章节主题或若干跨章主题下的实质子节点；不能只返回少量目录节点。标题用于浏览，摘要必须写出正文中的观点、事实、例子、过程、影响或结论，不能只重排目录。
...
- App 只保留基础结构校验：必须有且仅有一个根；父引用存在；无环、无孤立节点；标题和摘要非空。模型临时 ID 按前序重新分配为稳定 `mm001...`，层级和同级顺序由 App 计算。
- 不再要求模型回传并精确比对章节覆盖集合，也不因摘要措辞、节点密度或缺少 evidence 重试模型。结构化 JSON 无法解析、树拓扑非法或输出被截断时本轮明确失败，不在本地修 JSON。
