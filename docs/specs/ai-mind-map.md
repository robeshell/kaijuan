...
- 导图视觉从当前皮肤的语义主题派生，不保存为业务数据：根节点是唯一强强调面；一级分支按稳定顺序取 `primary` / `tertiary` / `secondary` 及其混合色，后代继承所属一级分支颜色。普通节点保持中性阅读卡片，只以低透明度染色、窄色轨和同色连线标识分支，不使用 `error` 等状态色拼成固定彩虹，也不让模型决定颜色。节点阴影只使用 `AppGlassTheme.shadow × AppSkinEffects.shadowScale`，深浅皮肤均需保持边框和文字对比。
- 节点文字区分“短主题”和“正文摘要”：主题使用语义 label 字号与中等字重，摘要使用语义 caption 字号、至少 `1.4` 行高并保持三行可读；根节点可以反白，非根节点保持中性主次文字色。装饰色不得承担唯一层级含义，层级列表与节点详情继续提供等价访问。
