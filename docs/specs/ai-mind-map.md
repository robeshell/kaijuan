...
- “本章”或“这本书/全书”本身就是完整范围确认，产品不提供章节勾选器或二次范围弹窗。整书使用思维导图自己的确定性正文策略：可以复用书籍结构事实，但不得复用知识图谱“全部展示、等待用户确认”的范围语义。版权、目录、参考文献等明确辅文与正文等同于标题的空容器单元在进入模型前自动移除；短小但确有正文的序言、结语或章节不能仅凭字符数删除。
...
- 多内容单元的完整输入仍在单次安全预算内时直接生成，不为“多章”本身强制增加 batch/reduce。确需分批时，最终结构化输出必须原样回传全部 `coveredSections`，App 对集合做精确校验；遗漏任一冻结章节的结果视为失败并携带缺失范围重试。整书最低节点密度同时考虑正文字符数与有效章节数，不能让三万字和九十万字范围共享同一固定下限。
