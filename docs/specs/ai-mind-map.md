...
- `title` 是短主题标签，`summary` 是用户直接阅读的正文总结；每个节点都必须有摘要，且摘要不能只复述标题或使用“主要内容”“相关内容”等占位话术。根摘要概括所选范围的中心结论，一级节点组织主要论点或叙事阶段，二级及更深节点承载原因、事实、例子、影响或结论。根节点不得把整段总结塞进标题。模型输出仍需经过长度、摘要有效性、层级、分支均衡、证据范围和引文定位校验。
...
- 结构校验失败后的重试必须携带 App 给出的具体、无正文内容的修复原因（例如层级不足或父引用无效），不得用相同提示盲目重复。模型以成功终态返回但 Genkit 无法解析 JSON 语法时，也属于可修复的模型输出失败：Adapter 只负责标记异常，Workflow 在同一阶段预算内重新请求合法完整 JSON；不得在本地补标点、解 fence、截取 JSON 或接受部分节点。层级口径统一为根节点 `level=0`，至少包含 `level=2` 的孙节点，最多到 `level=4`。根节点多余 evidence 由 App 丢弃。模型提供的引文必须落在冻结正文范围内，成功定位后启用“跳回原文”；模型漏传、改写或无法定位引文时只丢弃该引文，不得让摘要节点或整棵树失败，也不得从正文自由猜测补证。
- 批次主题数与最终树的最低结构密度由 App 从冻结正文字符数确定，并同时写入提示词与业务校验。短范围仍允许紧凑结果；达到长章节规模时最终至少需要 10 个节点和 3 个根分支，不能让模型用全局固定的最低线吞掉论证链。生成提示按 `contentKind` 要求实质覆盖：论说内容包含主张、理由、事实或例子与结论；叙事内容包含人物或事件、动机、转折与影响；知识型内容包含概念、定义、方法、条件与限制。单章需覆盖正文开头、中段和结尾，整书归并不得退化成章节标题目录。
...
- 支持缩放、自由拖动、重新居中、折叠/展开分支、节点详情和从证据跳回原文。画布拖动不得在导图外 240px 等近距离设置透明边界，应允许用户把任意分支自由移到视口中心；手动缩放下限为 `0.2`，避免把内容缩到不可操作。桌面鼠标滚轮与触控板滚动缩放必须以当前指针位置为锚点，缩放前后指针下的场景坐标保持不变。指针位于嵌入画布内时，外层对话列表必须暂停滚动以覆盖滚轮和触控板 pan/zoom 两类事件；指针离开后恢复。对话卡片提供“全屏查看”，以独立 `Scaffold` 路由铺满 App 工作区；它共享同一结构化数据和布局选择回调，不新增 Tab 或第二份缓存。
