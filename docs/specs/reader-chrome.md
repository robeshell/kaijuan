...
- **漫画引擎**：页图 chrome（模式 / 方向 / 页进度 / 缩略图 / 亮度）；页图交互见 [comic-reader.md](./comic-reader.md)。
...
- **图书**：点按区做在正文手势层内（非 Stack 盖层），避免抢脚注/内链：翻页模式左右约 **28%** 翻页、中间 **44%** 显隐 chrome；**chrome 已显示时任意空白点按只关 chrome、不翻页**；滚动模式点空白显隐 chrome。进度条 seek 用全书 `progressFraction`（Foliate loc），松手定位。
- **图书跨端输入**：移动端以触摸 / 手写笔直接操纵正文；桌面端同时支持触控板、滚轮和鼠标按住拖动。输入设备集合由图书阅读器统一策略提供，滚动 / 翻页视图不得各自分叉。正文链接与脚注点击优先于空白点按，拖动不触发 chrome 显隐。
- **漫画**：与图书同一套 **不透明表面 + 底栏工具条** 语言；底栏五键为缩略图 / 书签 / 亮度 / 方向 / 阅读模式（模式+主题）；中央单击显隐（未放大时）；**chrome 已显示时点按只关 chrome**；缩放仅 pinch（不设双击，避免拖慢单击）；双页对开贴合见 comic-reader spec。
- **移动端横屏（不锁方向）**：可用高度不足时（`appIsShortViewport`）压缩壳层与阅读 chrome——底栏图标化、顶栏单行、工具面板限高可滚、封面边距收紧；正文仍跟视口重排。
- 顶底栏用阅读主题实色（`blur: false`），保证工具条可读；跟随阅读主题深浅。  
