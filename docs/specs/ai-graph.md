...
- 抽取输出先视为 mention。`identityHint`（身份/角色提示）、本章关系和证据共同参与 mention → entity 的消歧；它不是跨章主键，**不得**编进唯一同名实体的 `entityId`。只有同一文本单元出现多个同名同类行时，才把 hint 编入新 ID 以拆开同名异人。同一单元多个同名同类行若共享明确别名则视为重复 mention 并融合。跨文本单元仅有一个同名候选时，即使提示措辞漂移也复用已有 ID（`type + name`）。
...
- **LLM 只输出 `sectionIndex + quote`**；`progressInSection` 由程序在原文中做归一化搜索（去空白、统一标点/全半角/大小写）后回填。同一引文在章内出现多次时，优先取靠近该实体名或别名（关系则靠近两端名称）的那一处，避免进度钉在第一次巧合命中。
- 过短引文（去标点后不足 4 字）且引文本身不含实体名/别名时视为未定位，防止「他说」这类碎片误匹配。
...
输入：用户确认的 sections（细粒度逻辑单元）+ 已保存的生成范围
  ...
  → 连续短章按字数打包成一次抽取（目标约 8,000 字，上限 36,000）；一章本身已够长则仍单独一次
  → 空节、不足约 80 字的尾巴并进相邻包，不单独打模型
  → 不跨未选中的缺口、不跨作品；覆盖仍按原章节写入 coveredSections
  → 只有把多章塞进同一 spine 文档、单包仍超过 36,000 字时才按 8,000 字再切
  → 最多 3 包并行（超时不得拆掉共用连接）；单章每块最多 16 实体 / 16 关系，打包时略增（上限 24）
  → JSON 无法解析或截断：多章包先按章对半拆；单章再对半切一次字串
  → 漏项复核：完全没有人物时补全部缺类；已有人物时，只在展示方案强调的组织/地理整类为零时补一次
  → 描述润色：占位句（「尚未登场」等）或后文已有新证据而描述仍停在首次出场时重写；增量只润色本轮刚补过证据的实体
  → 增量续跑只抽未覆盖节，亲属方向只核本轮新证据
  → 程序回填 quote → progressInSection
  → 顺序增量合并（见 4.2）→ 落盘 ai_graph/$hash.json
  → onProgress（completed/total = 章节数；label 含当前节标题，增量续跑用「接着分析」）
```

- **输出预算**：一次调用对应一个字数包，不是「目录上的一章」。输出仍按大约 2,048–2,560 token，避免一包写成超大 JSON 把模型拖死。坏 JSON / 截断先拆包再切字。
- **并行**：最多 3 包同时抽；状态条写包内首尾章名（`第一章–第五章`）。单路超时不得关闭共用 HTTP 连接。用户点停止才撤销整次运行。
- **收尾**：有人物且展示方案未强调的类型有缺漏时，不跑全书漏项复核。描述仍是占位句、或后文证据已经越过 `descriptionSection` 时，才对高频实体做一次有界润色。短章很多的小书靠打包把调用次数压到和长章书同一量级，不能 40 个目录项打 40 次。
- **增量成本**：已有覆盖记录时不跑 step-0 / 漏项；润色只覆盖本轮刚写入新证据、且描述已过期的实体。进度刷新不得拆实体列表或重绘正文。
- **调用预算**：按正文长度估算块数（每块最多再试 1 次）+ 固定收尾次数，不用「节数 × 12」这种与字数无关的上限。
...
3. **描述更新**：全部抽取与身份归并后，对「占位句」或 `lastSection > descriptionSection` 的实体，按累计证据重写读者可见描述；该步骤失败时保留已经生成的图谱和旧描述。增量续跑只处理本轮刚补过证据的实体。
...
- 每条实体与关系**必须携带原文证据**（`sectionIndex + quote`）；`progressInSection` 由程序回填（§3.3）。多命中优先对齐到名称附近；过短且引文不含名称的碎片不算已定位。
...
- 「分裂」（同一人拆成两个节点）靠别名归并缓解，用户也可在实体卡把两个同类型实体手动并成一个；「误并」（两人并成一个节点）靠歧义拒绝避免。手动拆分尚未实现。
...
- 实体卡可把两个同类型实体手动合并（被并入方变成别名，关系与出处转到留下的一方，写入 `mergeLog`，`reason=manual`）。
- 按稳定 ID 隐藏实体，保留在图谱包的 `hiddenEntityIds` 中；「已隐藏的实体」可逐条恢复显示。手动拆分尚未实现。
...
本书 AI → 知识图谱
  ...
  → 生成中：当前节标题 + 章节批次进度 +「停止」（保留已完成章节）；状态条固定在图谱页签顶部、位于滚动内容之外，手机滚动位置、空快照或旧内容都不得遮掉它
  → 已生成：顶部范围 badge + 视图切换（实体列表 | 关系图）
       ...
       → 实体卡底部：合并到…（同类型实体）/ 从图谱中隐藏
       → 顶部更多：已隐藏的实体（恢复显示）；删除图谱
       → 顶部操作：重新生成（= 删除重建）
```
...
- **手动修正**：实体卡可将当前实体并入另一个同类型实体（留下的一方保留规范名，被并入方成为别名；关系和出处改挂到留下的一方；自环边丢弃）。隐藏按稳定 ID 写入 `hiddenEntityIds`，不删数据；「已隐藏的实体」可恢复。跨类型不合并。手动拆分仍未实现。
- **生成进度**：状态条 `label` 带当前节标题（`正在分析 · 第五章`）；增量续跑用 `接着分析` / `接着从「第五章」继续`，避免看起来像从头开始。状态条右侧已有 `completed / total`，label 不再重复「第 x / n 节」。
...
| 生成中 | 页签顶部固定状态条显示当前阶段、正在分析的章节标题与序号 +「停止」；增量续跑用「接着分析」；状态条不随内容滚动（停止后保留已完成章节） |
| 已生成但为空 | 不显示全 0 索引；提示“尚无有效图谱数据”，允许直接重新抽取，旧 `coveredSections` 不得跳过正文 |
| 实体都已隐藏 | 不走空快照；提示“实体都已隐藏”，提供「已隐藏的实体」恢复入口，不要求重新生成 |
...
| 用户手动合并 | 图谱包内实体/关系 + `mergeLog`（`reason=manual`） | 同上 |
...
lib/presentation/controllers/book_ai_graph_controller.dart — 生成生命周期：缓存、作品切换、检查点、隐藏/合并/恢复/删除
