...
- 图书思维导图使用独立 `BookMindMapWorkflow` + `AiBookMindMapStore`，但产品入口和结果承载属于本书对话。对话层只把带生成动作的自然语言确定性路由为“当前章”或“当前作品/整书”，范围词本身就是确认，不增加章节选择器：“当前章”在发起时直接冻结 Foliate 当前文档正文与定位，不用阅读器 section 序号去过滤整书逻辑语料；Foliate 以可见文本提取并保留标题、段落和列表换行，不能用会粘连块级内容的裸 `textContent`。整书只复用公共书籍结构事实，使用思维导图自身的保守规则自动移除明确辅文和纯标题容器，不调用 `AiGraphScopePlanner` 也不读取知识图谱的排除设置。冻结内容在单批字符预算内直接经 `AiWorkflowModelSession.completeJson` 生成树，不经 batch/reduce；只有超出预算时才均衡采样、分批汇总并原子 checkpoint，最终 schema 必须精确回传全部冻结 section ID。结构化节点中的 `title` 只是短主题标签，`summary` 是必须由正文提炼出的用户可见内容，需按内容类型覆盖论点与论据、事件与影响，或概念与适用条件；校验器拒绝空摘要和只复述标题的占位摘要，原生画布在节点卡片内直接渲染摘要。最终校验同时负责树拓扑、长度、范围、结构密度和摘要有效性；引文有则严格定位并启用跳回原文，无则保留摘要节点，不得将知识图谱式的逐叶证据要求变成任务失败条件。对话卡片的“全屏查看”必须 push 覆盖工作区的 `Scaffold` 路由，不能把画布继续限制在聊天 side sheet 的固定高度；布局修改仍经回调写回对话附件。桌面 `InteractiveViewer` 使用无限 `boundaryMargin` 提供自由画布拖动，手动缩放下限为 `0.2`；它必须把触控板两指滚动作为平移，双指捏合才缩放并保持指针下的场景坐标不变。嵌入画布除占用 `PointerSignalResolver` 外，还必须在指针悬停期间把外层对话列表切为不可滚动，以覆盖 macOS 触控板 pan/zoom 手势；指针离开后恢复列表滚动。它不依赖 `AiBookGraph` 或 Mermaid；通用聊天 Mermaid 仍走富内容渲染。布局算法和画布位于纯 Dart/Flutter 表现边界，Genkit、SVG/HTML 与模型临时 ID 不进入业务缓存。节点与连线的视觉映射也只属于表现边界：根据确定性拓扑和当前 `ColorScheme` / `AppGlassTheme` / `AppSkinEffects` 派生分支色、表面与阴影，不写入模型、checkpoint 或对话附件。详见 [ai-mind-map.md](./specs/ai-mind-map.md)。
