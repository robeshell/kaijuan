...
- 图书思维导图使用独立 `BookMindMapWorkflow` + `AiBookMindMapStore`。Workflow 接收 App 冻结的 `contentHash/workKey/sectionIndices`，经 `AiWorkflowModelSession.completeJson` 分批生成并原子 checkpoint，完成后保存扁平稳定节点；不依赖 `AiBookGraph`、聊天消息或 Mermaid。布局算法和画布位于纯 Dart/Flutter 表现边界，Genkit、SVG/HTML 与模型临时 ID 不进入业务缓存。详见 [ai-mind-map.md](./specs/ai-mind-map.md)。
