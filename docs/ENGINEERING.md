...
- 图书思维导图使用独立 `BookMindMapWorkflow` + `AiBookMindMapStore`，但入口和结果承载属于本书对话。对话层只把带生成动作的自然语言确定性路由为“当前章”或“整本出版物”：当前章直接冻结 Foliate 当前文档正文与定位，整书不先调用作品识别，不为同一请求重复提取整书语料。Foliate 以可见文本提取并保留标题、段落和列表换行。整书仅使用思维导图自身的保守规则移除明确辅文和纯标题容器，不调用 `AiGraphScopePlanner` 或知识图谱范围设置。内容在单批安全预算内时直接经 `AiWorkflowModelSession.completeJson` 生成树；只有超出预算才均衡采样、使用较大批次汇总并原子 checkpoint，最终 schema 必须精确回传全部冻结 section ID。节点数、同级分支数和层级深度不设产品配额；校验器只保留合法树拓扑、精确范围覆盖、非空且有实质内容的摘要，以及可选引文的范围与定位。引文缺失不使节点或整次任务失败。对话卡片与全屏画布共享同一结构化产物和布局回调；桌面画布使用自由边界、`0.2` 手动缩放下限、触控板两指平移与捏合缩放，嵌入画布在指针悬停期间暂停外层对话滚动。它不依赖 `AiBookGraph` 或 Mermaid；通用聊天 Mermaid 仍走富内容渲染。布局、画布和主题视觉映射位于纯 Dart/Flutter 表现边界，Genkit、SVG/HTML、模型临时 ID 和派生颜色不进入业务缓存。详见 [ai-mind-map.md](./specs/ai-mind-map.md)。
