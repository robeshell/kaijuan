...
| 对话产品工具 | 未附加产物的自由输入只进入 `AiChatService` 的同一受控模型回合；除五个只读书内工具外，App 按本轮上下文动态声明 `create_book_mind_map` / `revise_book_mind_map`。模型不调用产品工具时正常回答；调用时必须作为该回合唯一的终止工具请求，`AiRunOrchestrator` 发出类型化产品行动事件，由 Controller 再校验并启动确定性 Workflow。若模型未调用产品工具却在正文中明确声称已交付原生思维导图，`AiChatService` 必须撤回该草稿并进行至多一次仅暴露产品工具、`toolChoice=required` 的协议修复；仍无合法调用则失败，禁止把 Markdown 提纲持久化为成功回答。快捷按钮、显式产物附件和失败重试直接携带 App 已知的结构化命令。“继续修改”把真实 `artifactId` 附加到输入框；附件存在时，下一条及后续输入直接进入该产物的确定性修订链，关闭附件才恢复普通对话，不再请求模型重复判断已由 UI 明确的动作和目标。普通模型回合只暴露 `artifact_1` / `work_1` 等临时别名、修订号、相邻/优先/当前标记，不暴露可伪造的数据库 ID；人类可读标题是转义后的不可信匹配数据。原生附件以 `artifactId / sourceArtifactId / revision` 追踪谱系；普通 Mermaid 不进入产物目录、不解析、不导入 | 不增加前置意图模型、自由文本关键词路由、标题包含匹配、`targetMode` 规则矩阵或第二套 transport。协议修复只校验模型自己的伪交付正文，不据此猜用户范围；修复回合仍需通过产品别名解析和 Controller 冻结快照校验。产品工具输出不是执行授权；Controller 必须使用发送前冻结的章节、作品、manifest、会话 `workKey` 和产物目录重新映射别名，校验同书范围、预算、取消和运行状态。显式附件也只能提交冻结的真实产物身份，不能绕过 Controller 修订预检。读工具与产品终止工具不得在同一模型响应中混用。普通聊天 Mermaid 和图书原生导图保持独立 |
