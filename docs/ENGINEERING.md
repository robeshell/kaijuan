...
| 对话产品工具 | 自由输入只进入 `AiChatService` 的同一受控模型回合；除五个只读书内工具外，App 按本轮上下文动态声明 `create_book_mind_map` / `revise_book_mind_map`。模型不调用产品工具时正常回答；调用时必须作为该回合唯一的终止工具请求，`AiRunOrchestrator` 发出类型化产品行动事件，由 Controller 再校验并启动确定性 Workflow。快捷按钮和失败重试直接携带 App 已知的结构化命令。每轮只向模型暴露临时产物别名、修订号、相邻/优先标记，不暴露可伪造的数据库 ID；“继续修改”只设置优先别名，不劫持后续任意输入。原生附件以 `artifactId / sourceArtifactId / revision` 追踪谱系；普通 Mermaid 不进入产物目录、不解析、不导入 | 不增加前置意图模型、自由文本关键词路由、`targetMode` 规则矩阵或第二套 transport。产品工具输出不是执行授权；Controller 必须重新映射别名、校验同书范围、预算、取消和运行状态。读工具与产品终止工具不得在同一模型响应中混用。普通聊天 Mermaid 和图书原生导图保持独立 |
