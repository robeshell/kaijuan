...
- 图书思维导图使用独立 `BookMindMapWorkflow` + `AiBookMindMapStore`，但产品入口和结果承载属于本书对话。对话层只把自然语言确定性路由为“当前章”或“当前作品/整书”：“当前章”在发起时直接冻结 Foliate 当前文档正文与定位，不用阅读器 section 序号去过滤整书逻辑语料；Foliate 以可见文本提取并保留标题、段落和列表换行，不能用会粘连块级内容的裸 `textContent`。单个冻结内容单元在单批字符预算内直接经 `AiWorkflowModelSession.completeJson` 生成树，不经 batch/reduce；作品/整书跨多个单元时才均衡采样、分批汇总并原子 checkpoint。最终校验只负责树拓扑、长度、范围和结构密度；引文有则严格定位并启用跳回原文，无则保留摘要节点，不得将知识图谱式的逐叶证据要求变成任务失败条件。它不依赖 `AiBookGraph` 或 Mermaid；通用聊天 Mermaid 仍走富内容渲染。布局算法和画布位于纯 Dart/Flutter 表现边界，Genkit、SVG/HTML 与模型临时 ID 不进入业务缓存。详见 [ai-mind-map.md](./specs/ai-mind-map.md)。
