...
- 图书思维导图使用独立 `BookMindMapWorkflow` + `AiBookMindMapStore`，但产品入口和结果承载属于本书对话。对话层只把自然语言确定性路由为“当前章”或“当前作品/整书”：“当前章”在发起时直接冻结 Foliate 当前文档正文与定位，不用阅读器 section 序号去过滤整书逻辑语料；Foliate 以可见文本提取并保留标题、段落和列表换行，不能用会粘连块级内容的裸 `textContent`。单章正文在单批字符预算内完整送入提炼，超限才做头中尾保序采样。作品/整书才冻结 `contentHash/workKey/sectionIndices` 并跨章节均衡采样。Workflow 经 `AiWorkflowModelSession.completeJson` 分批生成并原子 checkpoint，完成后把扁平稳定节点作为结构化对话附件持久化；批次主题数和最终树密度根据冻结正文规模设下限，拒绝长正文退化成 schema 下限提纲。它不依赖 `AiBookGraph` 或 Mermaid；通用聊天 Mermaid 仍走富内容渲染。布局算法和画布位于纯 Dart/Flutter 表现边界，Genkit、SVG/HTML 与模型临时 ID 不进入业务缓存。详见 [ai-mind-map.md](./specs/ai-mind-map.md)。
