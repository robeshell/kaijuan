...
| 对话意图路由 | 当前思维导图入口由 `resolveAiMindMapRequestScope` 做确定性识别；迁移目标是 `AiConversationRouter` 返回 `object / action / scope / target / modifiers`，并分为普通聊天、Workflow 命令和缺参确认三类 | 路由层不调用模型、不读取正文、不决定真实书籍范围；Widget 不继续堆叠业务关键词；低置信度只能追问或回落普通聊天 |
