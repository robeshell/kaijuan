...
- 图书思维导图是本书对话的直接结构化生成，由 `AiBookMindMapService` 经 `AiWorkflowModelSession.completeJson` 完成一次所选范围调用。入口只匹配窄生成命令并保留用户原始提示；调用前复用 `AiBookStructureSession`：当前章冻结 Foliate 文档，普通单书一次发送完整有效正文，`segmentedSingleWork` 按既有分部/分卷范围依次生成，`multiWorkOmnibus` 和带候选作品的 `uncertain` 结构先由 UI 展示作品/章节统计并等待选择。正文只移除设置中明确的出版外围标题、版权/目录信号和纯标题容器，不做固定字符采样；供应商上下文不足时明确失败。模型只返回主题树 Schema，App 校验单根、父引用、环/孤儿和非空文本，重新分配稳定 ID，并把能逐字定位的可选引文转换为阅读器坐标。旧 `BookMindMapWorkflow` 的 batch/reduce、checkpoint、`AiBookMindMapStore`、`ai_mind_map/` 和重复 WebDAV记录全部删除；唯一持久化事实是 `ai_chat/` 消息附件。对话卡片与全屏画布继续共享同一产物和布局回调；普通聊天 Mermaid 与知识图谱保持独立。详见 [ai-mind-map.md](./specs/ai-mind-map.md)。
