...
| 对话意图路由 | `AiConversationRouter` 返回 `object / action / scope / target / modifiers`，并分为普通聊天、Workflow 命令和缺参确认三类；结构化导图附件由 App 分配 `artifactId` 并以 `sourceArtifactId + revision` 追踪编辑谱系 | 路由层不调用模型、不读取正文、不决定真实书籍范围；Widget 不继续堆叠业务关键词；低置信度只能追问或回落普通聊天；布局变更不得伪造新 artifact |
