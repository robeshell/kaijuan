...
class Reader {
  ...
  async open(file, cfi) {
    ...
    // view.init() is the single owner of initial navigation. Calling
...
// Kaika routes one EPUB to either the reflow or page-image engine. Sampling
// evenly across the spine avoids classifying boxed sets from cover/front-matter
// sections only, while keeping import work bounded for very large books.
const sampleSpineText = async (book, maxSections = 12) => {
  ...
  for (const index of indices) {
    ...
    try {
      ...
      console.warn('Failed to sample EPUB section', index, e)
    } finally {
      section.unload?.()
    }
  }
...
const getMetadata = async () => {
  ...
  const [cover, sample] = await Promise.all([
    book.getCover().then(blobToDataURL).catch(e => {
      console.warn('Failed to read EPUB cover', e)
      return null
    }),
